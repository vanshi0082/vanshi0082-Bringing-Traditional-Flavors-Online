...
if(empty($_SESSION["user_id"]))
{
    ...
    $userQuery = mysqli_query($db, "SELECT email, name FROM users WHERE id = '$user_id'");
    ...
    foreach ($_SESSION["cart_item"] as $item) {
        ...
        if($_POST['submit']) {
            ...
            try {
                ...
                $msg = '<h1>Hello, <b>' . htmlspecialchars($user_name) . '</b></h1>';
                $msg .= '<p>Thank you for your order. Here are the details:</p>';
                $msg .= '<table border="1" cellpadding="10" cellspacing="0" style="border-collapse: collapse;">';
                $msg .= '<thead><tr><th>Title</th><th>Quantity</th><th>Price</th></tr></thead><tbody>';

                foreach ($_SESSION["cart_item"] as $orderItem) {
                    $msg .= '<tr>';
                    $msg .= '<td>' . htmlspecialchars($orderItem['title']) . '</td>';
                    $msg .= '<td>' . htmlspecialchars($orderItem['quantity']) . '</td>';
                    $msg .= '<td>' . htmlspecialchars($orderItem['price']) . '</td>';
                    $msg .= '</tr>';
                }

                $msg .= '</tbody></table>';
                $msg .= '<p>Total Amount: ₹' . $item_total . '</p>';
                $msg .= '<p>We hope you enjoy your meal!</p>';

                $mail->isHTML(true);
                $mail->Subject = $subject;
                $mail->Body = $msg;
                $mail->AltBody = 'Thank you for your order. Your order has been placed successfully.';
